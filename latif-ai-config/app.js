...
$("btnAutoDetect").addEventListener("click", async () => {
  const statusEl = $("connStatus");
  const serversEl = $("detectedServers");
  statusEl.className = "conn-status show";
  statusEl.textContent = "🔍 Scanning for local model servers…";
  serversEl.style.display = "none";

  const result = await ServerAutoDetect.autoConnect();

  if (result.success) {
    const server = result.server;
    State.host = server.host;
    State.port = String(server.port);
    State.backend = server.backend;
    saveState();

    $("srvHost").value = State.host;
    $("srvPort").value = State.port;
    document.querySelectorAll("#backendSeg .seg-btn").forEach((b) =>
      b.classList.toggle("active", b.dataset.backend === State.backend)
    );
    if (State.backend === "openai") {
      $("openaiUrlRow").style.display = "flex";
    }

    statusEl.className = "conn-status show ok";
    statusEl.textContent = `✓ Auto-detected: ${server.name}`;
    toast(`✓ Connected to ${server.name}`);

    // Show other available servers if any
    if (result.servers && result.servers.length > 1) {
      serversEl.innerHTML = `<div style="font-size:12px; opacity:0.8; margin-top:12px;"><strong>Other servers available:</strong>
        ${result.servers.slice(1).map(s => `<div style="margin-top:4px;">• ${s.name} (${s.models.length} model(s))</div>`).join("")}
      </div>`;
      serversEl.style.display = "block";
    }
  } else {
    statusEl.className = "conn-status show fail";
    statusEl.textContent = `✗ ${result.error}`;
    toast("No local servers found. Install Ollama from https://ollama.ai");
  }
});
...
function init() {
  ...
  // Auto-detect on first load if no server configured
  const firstLoad = !localStorage.getItem("latif_first_load_done");
  if (firstLoad) {
    localStorage.setItem("latif_first_load_done", "true");
    ServerAutoDetect.tryConnectWithCache().then((result) => {
      if (result.success) {
        State.host = result.server.host;
        State.port = String(result.server.port);
        State.backend = result.server.backend;
        saveState();
      }
    }).catch(() => {});
  }
